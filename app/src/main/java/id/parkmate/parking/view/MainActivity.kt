...
import id.parkmate.parking.model.data.CountdownViewModel
...
import id.parkmate.parking.model.data.sessionmanager.Companion.CountDown
...
import kotlinx.android.synthetic.main.parkir.arc_progress
...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (savedInstanceState == null) {
            // Initialize countdown only if it's not already running
            restoreCountdown()
        } else {
            // Restore countdown value after rotation
            viewModel.countdownValue = savedInstanceState.getLong("countdownValue")
            startCountdown()
        }
...
//            overridePendingTransition(0, 0)
    ...
    private fun countdown() {
        val testcount = findViewById<TextView>(R.id.testcountdown)
        ...
        val resultTime = format.format(calendar.time)

        val endDate = format1.parse(resultTime)
        // milliseconds
        val different = endDate.time - currentTime.time
        viewModel.countdownValue = different

        startCountdown()
    }

    override fun onSaveInstanceState(outState: Bundle) {
        // Save the countdown value before rotation
        outState.putLong("countdownValue", viewModel.countdownValue)
        super.onSaveInstanceState(outState)
    }

    override fun onStop() {
        // Simpan endTimestamp ketika aplikasi dihentikan
        val sharedPreferences = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val editor = sharedPreferences.edit()
        editor.putLong("endTimestamp", viewModel.endTimestamp)
        editor.apply()
    ...
    private fun restoreCountdown() {
        val sharedPreferences = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
        val endTimestamp = sharedPreferences.getLong("endTimestamp", 0)

        if (endTimestamp > 0) {
            viewModel.setEndTimestamps(endTimestamp)

            // Hitung waktu yang tersisa dan mulai perhitungan mundur
            val currentTime = Calendar.getInstance().timeInMillis
            val remainingTime = endTimestamp - currentTime

            if (remainingTime > 0) {
                viewModel.countdownValue = remainingTime
                startCountdown()
            } else {
                // Handle kasus di mana perhitungan mundur sudah selesai
            }
        } else {
            // Tidak ada endTimestamp yang disimpan, mulai perhitungan mundur baru
            countdown()
        }
    }
    ...
    private fun startCountdown() {
        ...
        object : CountDownTimer(viewModel.countdownValue, 1000) {
            override fun onTick(millisUntilFinished: Long) {
                viewModel.countdownValue = millisUntilFinished

                val secondsInMilli: Long = 1000
                val minutesInMilli = secondsInMilli * 60

                val elapsedMinutes = millisUntilFinished / minutesInMilli
                val remainingSeconds = (millisUntilFinished % minutesInMilli) / secondsInMilli

                testcount.text = "Berlaku Selama : $elapsedMinutes Menit dan $remainingSeconds Detik"
            }
...
//
//                testcount.text = "Berlaku Selama : $elapsedMinutes Menit dan $elapsedSeconds Detik"
//            }
//
//            override fun onFinish() {
